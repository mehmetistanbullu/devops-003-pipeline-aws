    ...
    stages {
        ...
        stage('Checkout from SCM') {
            steps {
                ...
                git branch: 'master', credentialsId: 'GITHUB_TOKEN', url: 'https://github.com/mehmetistanbullu/devops-003-pipeline-aws'
            }
        }
        stage('Build Maven') {
            steps {
        ...
        stage('Trivy Scan') {
            steps {
                script {
                    sh('docker run -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy image istanbullumem/devops-003-pipeline-aws:latest --no-progress --scanners vuln  --exit-code 0 --severity HIGH,CRITICAL --format table')
                }
            }
        }
